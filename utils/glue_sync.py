...
def sync_jobs(postgres_instance, glue_instance, role_arn):
    ...
    postgres_instance.update_jobs_table_is_run('Y')

    # Get all relevant aws jobs from Postgres db
    df_job, e = postgres_instance.get_glue_jobs_from_db()
    if e is not None:
        postgres_instance.update_jobs_table_is_run('N')
        sys.exit(-1)
    ...
    df_glue_jobs, e = pd.DataFrame(glue_instance.get_glue_jobs(), columns=['job_name'])
    ...
    for _, row in df_delete_recs.iterrows():
        ...
        e = glue_instance.delete_glue_job(row['job_name'])
        if e is not None:
            postgres_instance.update_jobs_table_is_run('N')
            sys.exit(-1)
    ...
    for _, row in df_insert_recs.iterrows():
        ...
        e = glue_instance.create_glue_job(
                row['job_name'],
                row['job_description'],
                role_arn,
    ...
    for _, row in df_update_recs.iterrows():
        ...
        e = glue_instance.update_glue_job(
                row['job_name'],
                row['job_description'],
                role_arn,
                row['job_param']['script_location'],
