...
def validate_input_table_with_metadata(postgres_instance, job_name, job_instance, target_tables):
    ...
    actual_count_from_db = postgres_instance.check_table_exists(job_name, job_instance, tables_formatted.strip(','))
...
def run_glue_job(job_name, job_instance, postgres_instance, glue_instance, tables=None, max_dpu=None, incr_from="",
    ...
    if job_run_id is not None:
        if status in ['STARTING', 'RUNNING', 'STOPPING', 'UNKNOWN']:
            job_status = glue_instance.get_glue_job_status(job_name, job_run_id)
    ...
    job_run_id = glue_instance.start_glue_job(job_name, job_instance, target_tables, max_dpu, incr_from, incr_to,
                                              context)
    # job_run_id = "XYZ123498"
    postgres_instance.update_job_instance(job_name, job_instance, job_run_id, 'UNKNOWN')

    while True:
        glue_job_status = glue_instance.get_glue_job_status(job_name, job_run_id)
