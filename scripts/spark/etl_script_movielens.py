...
def get_job_args():
    return getResolvedOptions(sys.argv, ['JOB_NAME', 'job-instance', 'tables', 'incr-from', 'incr-to', 'job-context'])
...
def glue_transformations(spark, table):
    ...
    df = spark.read.option("header", False).option("inferSchema", False).csv(
        "*.csv")
